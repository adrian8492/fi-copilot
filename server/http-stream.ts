...
function createDeepgramConnection(state: StreamSession) {
  ...
  connection.on(LiveTranscriptionEvents.Transcript, async (data) => {
    ...
    // Persist to DB with retry — track failures so grading knows data may be incomplete
    ...
    if (!inserted) {
      ...
      console.error(`[HTTP-Stream] Transcript lost for session ${state.sessionId} (${state.failedInserts} total failures)`);
      broadcast(state, "error", { message: `Transcript failed to save (${state.failedInserts} lost). Grading may be incomplete.` });
    }
      ...
      if (quickSuggestion) {
        ...
        try {
          await insertCopilotSuggestion({
            ...
            script: quickSuggestion.script as string | undefined,
            framework: quickSuggestion.framework as string | undefined,
            scriptId: quickSuggestion.scriptId as string | undefined,
          ...
          if (llmSugg) {
            try {
              await insertCopilotSuggestion({
                ...
                scriptId: (llmSugg as Record<string, unknown>).scriptId as string | undefined,
...
export function createHttpStreamRouter(): Router {
  ...
  router.post("/audio", (req: Request, res: Response) => {
    ...
    try {
      ...
      if (readyState === 1) {
        ...
        if (state.audioBuffer.length >= 50) {
          console.warn(`[HTTP-Stream] Audio buffer overflow (50 chunks) for session ${state.sessionId} — dropping audio`);
          broadcast(state, "error", { message: "Audio buffer full — some audio is being dropped. Transcription may have gaps." });
        } else if (state.audioChunkCount <= 5) {
          console.warn(`[HTTP-Stream] Deepgram not ready (state: ${readyState}) — dropping chunk`);
        }
      }
    } catch (err) {
        ...
        if (quickSugg) {
          ...
          try {
            await insertCopilotSuggestion({
              ...
              script: quickSugg.script as string | undefined,
              framework: quickSugg.framework as string | undefined,
              scriptId: quickSugg.scriptId as string | undefined,
  ...
  router.post("/end", async (req: Request, res: Response) => {
    ...
    // Drain: give Deepgram up to 1.5s to flush final transcripts before cleanup.
    if (state.deepgramConnection) {
      try { state.deepgramConnection.requestClose(); } catch { /* ignore */ }
      await new Promise((resolve) => setTimeout(resolve, 1500));
    }

    broadcast(state, "session_ended", { sessionId: state.sessionId, durationSeconds: finalElapsed, failedInserts: state.failedInserts ?? 0 });
    ...
    console.log(`[HTTP-Stream] Session ${state.sessionId} ended after ${finalElapsed}s (${state.failedInserts ?? 0} failed inserts)`);
