        ...
        if (llmSuggestion) {
          try {
            await insertCopilotSuggestion({
              ...
              scriptId: (llmSuggestion as Record<string, unknown>).scriptId as string | undefined,
    ...
    ws.on("message", async (raw: Buffer | string, isBinary: boolean) => {
      ...
      if (isBinary) {
        ...
        try {
          ...
          if (readyState === 1) {
            ...
            // Buffer full or Deepgram in bad state — audio is being lost
            if (state.audioBuffer.length >= 50) {
              console.warn(`[WS] Audio buffer overflow (50 chunks) for session ${state.sessionId} — dropping audio`);
              send({ type: "error", message: "Audio buffer full — some audio is being dropped. Transcription may have gaps." });
            } else {
      ...
      switch (msg.type) {
        ...
        case "end_session": {
          ...
          // The transcript listener is still active on `state` during this window.
          if (state.deepgramConnection) {
          ...
          send({ type: "session_ended", data: { sessionId: state.sessionId, durationSeconds: finalElapsed, failedInserts: state.failedInserts ?? 0 } });
          console.log(`[WS] Session ${state.sessionId} ended after ${finalElapsed}s (${state.failedInserts ?? 0} failed inserts)`);
          break;
        }
      }
    });
